	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < spliceIndex.size()-1; i++) { // five index 
			for(int j = (i+1); j < spliceIndex.size(); j++) { // three index
				...
				if(validPath(tempPath, given.length(), spliceIndex.get(i), spliceIndex.get(j))) {
					double tempProb = getProbability(given.toCharArray(), given.length(), spliceIndex.get(i), spliceIndex.get(j));
					if(tempProb != 0) { /* remove zero probabilities */
		...
		/* select best path */
		System.out.println("Best: (" + bestPath + ", " + highestProb + ")");	
	}

	private static boolean validPath(String tempPath, int gLength, Integer five, Integer three) {
		if(((three - five) <= 1)) {
			return false;
